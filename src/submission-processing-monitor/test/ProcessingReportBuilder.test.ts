...
describe('buildProcessingReport', () => {
  test('a COMPLETED run includes counters and a problem per non-succeeded regular result', () => {
    const results: ProcessingResult[] = [
      { objectUuid: 'uuid-1', objectIndex: 1, objectType: 'submission', registrationAt: '2026-08-27', status: 'SUCCEEDED', reference: 'OF-1' },
      { objectUuid: 'uuid-2', objectIndex: 1, objectType: 'submission', registrationAt: '2026-08-27', status: 'FAILED', reference: 'OF-2', clientNumber: '12345' },
    ];
  ...
  test('excludes ESF problems - those belong to the separate ESF report', () => {
    const results: ProcessingResult[] = [
      { objectUuid: 'uuid-1', objectIndex: 1, objectType: 'esftaak', registrationAt: '2026-08-27', status: 'FAILED', esfStatus: 'afgerond', reference: 'ESF-1' },
    ];
  ...
  test('an INCOMPLETE run carries no counters or problems, even when results are passed in', () => {
    ...
    const results: ProcessingResult[] = [
      { objectUuid: 'uuid-1', objectIndex: 1, objectType: 'submission', registrationAt: '2026-08-27', status: 'FAILED' },
    ];
  ...
  test('a COMPLETED run with zero problems still reports its counters', () => {
    const results: ProcessingResult[] = [
      { objectUuid: 'uuid-1', objectIndex: 1, objectType: 'submission', registrationAt: '2026-08-27', status: 'SUCCEEDED' },
    ];
...
describe('buildEsfProcessingReport', () => {
  test('a COMPLETED run includes ESF counters and a problem per non-succeeded afgerond ESF result, with only reference/clientNumber/status', () => {
    ...
    expect(report.status).toBe('COMPLETED');
    expect(report.esfCounters).toEqual({ open: 1, verwerkt: 1, gesloten: 1, afgerond: 1, afgerondSucceeded: 1, afgerondProblem: 0 });
  ...
  test('excludes regular problems - those belong to the separate regular report', () => {
    const results: ProcessingResult[] = [
      { objectUuid: 'uuid-1', objectIndex: 1, objectType: 'submission', registrationAt: '2026-08-27', status: 'FAILED', reference: 'OF-1' },
    ];
  ...
  test('an INCOMPLETE run carries no ESF counters or problems, even when results are passed in', () => {
    ...
    const results: ProcessingResult[] = [
      { objectUuid: 'uuid-1', objectIndex: 1, objectType: 'esftaak', registrationAt: '2026-08-27', status: 'FAILED', esfStatus: 'afgerond' },
    ];
