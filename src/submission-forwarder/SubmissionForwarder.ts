...
export class SubmissionForwarder extends Construct {
  ...
  constructor(
    scope: Construct,
    id: string,
    private readonly options: SubmissionForwarderOptions,
  ) {
    super(scope, id);

    this.backupBucket = this.setupBackupBucket();
    this.esbQueue = this.setupEsbQueue();
    this.submissionTopic = this.setupSubmissionTopic();
  ...
  private setupParameters() {
    ...
    new Secret(this, 'documentenApiClientSecret', {
      description:
        'Client secret used by submission-forwarder to authenticate at documenten API',
    });

    new StringParameter(this, 'documentenApiClientId', {
      stringValue: 'submission-forwarder',
      description:
        'Client ID used by submission-forwarder to authenticate at documenten API',
    });
    ...
    const objectsApikey = new Secret(this, 'objects-apikey', {
      description:
        'API key used by submission forwarder for authentication at Objects API',
    });

    const documentenApiBaseUrl = new StringParameter(
      this,
      'documentenApiBaseUrl',
      {
        stringValue: '-',
        description:
          'Base URL used by submission-forwarder to reach the documenten API',
      },
    );
    const zakenApiBaseUrl = new StringParameter(this, 'zakenApiBaseUrl', {
      ...
      stringValue: '-',
      description:
        'Base URL used by submission-forwarder to reach the zaken API',
    });
    const catalogiApiBaseUrl = new StringParameter(this, 'catalogiApiBaseUrl', {
      ...
      stringValue: '-',
      description:
        'Base URL used by submission-forwarder to reach the catalogi API',
    });

    const mijnServicesOpenZaakApiClientId = new StringParameter(
      this,
      'mijnServicesOpenZaakApiClientId',
      {
        ...
        stringValue: 'submission-forwarder',
        description:
          'Client ID used by submission-forwarder to authenticate at mijn services open zaak APIs',
      },
    );

    const mijnServicesOpenZaakApiClientSecret = new Secret(
      this,
      'mijnServicesOpenZaakApiClientSecret',
      {
        secretName: `${baseParameterName}mijn-services-open-zaak/client-secret`,
        description:
          'Client secret used by submission-forwarder to authenticate at mijn services open zaak APIs',
      },
    );
    ...
    const receiver = new ReceiverFunction(this, 'receiver', {
      ...
      environment: {
        ...
        MIJN_SERVICES_OPEN_ZAAK_CLIENT_SECRET_ARN:
          this.parameters.mijnServicesOpenZaakApiClientSecret.secretArn,
  ...
  private setupDocumentStorageLambda() {
    ...
    const s3StorageFunction = new DocumentsToS3StorageFunction(
      this,
      'docsToS3',
      {
        memorySize: 1024,
    ...
    this.parameters.mijnServicesOpenZaakApiClientId.grantRead(
      s3StorageFunction,
    );
    this.parameters.mijnServicesOpenZaakApiClientSecret.grantRead(
      s3StorageFunction,
    );
  ...
  private setupOrchestrationStepFunction(
    ...
    const stepfunction = new StateMachine(this, 'orchestrator', {
      ...
      definitionBody: DefinitionBody.fromFile(
        'src/submission-forwarder/orchestration.asl.json',
      ),
    ...
    stepfunction
      ...
      .createAlarm(this, 'execution-failed-alarm', {
        ...
        alarmName:
          'submission-orchestrator-failed-alarm' +
  ...
  private setupNotificationMailLambda() {
    const accountHostedZoneName = StringParameter.valueForStringParameter(
      this,
      Statics.accountRootHostedZoneName,
    );
    const internalNotificationMailLambda = new InternalNotificationMailFunction(
      this,
    ...
    internalNotificationMailLambda.addToRolePolicy(
      new PolicyStatement({
        resources: ['*'],
...
class ForwarderParameters extends Construct {
  ...
  addForwarderParameters() {
    return new StringParameter(this, 'objectTypes', {
      ...
      description:
        'name##url pairs (semi-colon-separated) defining supported objecttypes',
    });
  }
}
