...
describe('handler', () => {
  ...
  it('sends email and traces success', async () => {
    ...
    expect(bodyToCheck).toContain('<\\\\server\\path\\to\\folder\\TestForm-REF123>'); // Double slashes because it escapes the slashes in the string
    expect(bodyToCheck).toContain('<\\monitor\\path\\TestForm-REF123>'); // Double slashes because it escapes the slashes in the string
  });
